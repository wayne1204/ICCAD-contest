...
Polygon* Layer::split_Y(Polygon* &bigGG,int y,bool is_top,Polygon* & inserted){
    cout<<"split y "<<endl;
...
Polygon* Layer::split_X_left(Polygon* &bigGG, int x_left,int x_right ,Polygon* &  inserted){
    ...
    //把左邊的指回來 
    for(tp=new_poly_left->get_bl();tp->get_tr()==bigGG;tp=tp->get_rt())
        tp->set_tr(new_poly_left);
    //把上面的接回來
    for(tp=bigGG->get_rt();tp->_bottom_left_x()>=x_left;tp=tp->get_bl())
        ;
    new_poly_left->set_rt(tp);
    for(;tp->get_lb()==bigGG;tp=tp->get_bl())
        tp->set_lb(new_poly_left);
    //下面指回來
...
Polygon* Layer::split_X_right(Polygon* &bigGG, int x_left,int x_right ,Polygon* &  inserted){
    ...
    for(tp=bigGG->get_tr();tp->get_bl()==bigGG;tp=tp->get_lb())
        tp->set_bl(new_poly_right);
    bigGG->set_tr(new_poly_right);
